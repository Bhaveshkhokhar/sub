...
//       if (userId && authToken && phoneNumber) {
...
// Optional: Add a separate function for token validation
export const validateAuthToken = async (token: string): Promise<boolean> => {
  try {
    const response = await fetch('https://db.subspace.money/v1/graphql', {
      ...
      body: JSON.stringify({
        query: `
          query ValidateToken {
            __typename
            wGetInfoV3(request: {lang: "en", version: 10}) {
              __typename
              isBlocked
            }
          }
    ...
    // If we get data back without errors, token is valid
    return !data.errors && data.data?.wGetInfoV3;
  } catch (error) {
...
// Optional: Enhanced auth check with token validation
export const checkAuthStatusWithValidation = async (): Promise<boolean> => {
  const basicAuthCheck = await checkAuthStatus();
  
  if (!basicAuthCheck) return false;
  
  // If basic auth passed, optionally validate token
  const { user } = useAuthStore.getState();
  if (user?.auth_token) {
    const isTokenValid = await validateAuthToken(user.auth_token);
